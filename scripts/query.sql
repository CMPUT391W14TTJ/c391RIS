...
SELECT r.record_id, p.full_name, d.full_name as doctor_name, r.full_name as radiologist_name, r.test_type, r.prescribing_date, r.test_date, r.diagnosis, r.description FROM radiology_record r, persons p, persons d, persons r WHERE r.patient_id = p.person_id AND r.doctor_id = d.person_id AND r.radiologist_id = r.person_id AND( contains(p.first_name, 'a', 01) > 0 OR contains(p.last_name, 'a', 02) > 0 OR contains(r.diagnosis, 'a', 03) > 0 OR contains(r.description, 'a',04 ) > 0);
