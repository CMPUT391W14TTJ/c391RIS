...
<BODY>
<?php
	function idInUsers($con, $id){
		$sql = "SELECT person_id FROM users WHERE person_id =" . $id;
		$parseid = oci_parse($con, $sql);
		oci_execute($parseid);
		return oci_fetch($parseid);
	}

	function validateid($con, $id) {
		$sqlid = "SELECT person_id as ID FROM persons WHERE person_id =" . $id;
		$parseid = oci_parse($con, $sqlid);
		oci_execute($parseid);
		if(!(oci_fetch($parseid))) {
	...
	if (isset($_POST['ChangeUsers'])) {
		if(idInUsers($conn, $_POST['oldPersonID']))) {			
/*
