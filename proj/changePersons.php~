	...
	function validateform() {
			if(!(empty($_POST['newFirstName']))) {
				if(strlen($_POST['newFirstName'])>24) {
					...
					$_SESSION['ump_ERRMSG'] .= "First name must be less than 24 characters \n";
				}
			}
			if(!(empty($_POST['newLastName']))) {
				if(strlen($_POST['newLastName'])>24) {
					$_SESSION['ump_ERR'] = TRUE;
					$_SESSION['ump_ERRMSG'] .= "Last name must be less than 24 characters \n";
				}
			}			
			if(!(empty($_POST['newAddress']))) {
				if(strlen($_POST['newAddress'])>128) {
					$_SESSION['ump_ERR'] = TRUE;
					$_SESSION['ump_ERRMSG'] .= "Address must be less than 128 characters \n";
				}
			}
			if(!(empty($_POST['newEmail']))) {
				if(strlen($_POST['newEmail'])>128) {
					$_SESSION['ump_ERR'] = TRUE;
					$_SESSION['ump_ERRMSG'] .= "Email must be less than 128 characters \n";
				}
			}
			if(!(empty($_POST['newPhone']))) {
				if(strlen($_POST['newPhone'])>10) {
					$_SESSION['ump_ERR'] = TRUE;
					$_SESSION['ump_ERRMSG'] .= "Phone must be less than 10 characters \n";
				}
			}
	}
